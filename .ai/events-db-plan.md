...
| `status` | TEXT | NOT NULL, DEFAULT 'active' | The event's status (e.g., `active`, `cancelled`). Indexed. |
| `audience` | TEXT | NOT NULL, CHECK(audience IN ('Public', 'MembersOnly')) | The target audience for the event. Indexed for filtering. |
...
| `status` | TEXT | NOT NULL, CHECK(status IN ('confirmed', 'waitlisted', 'cancelled')) | The user's current status for the event. |
...
To avoid feature overlap and simplify the domain logic, the `reservations_reservations` table will be modified. The concepts of "public" or "joinable" reservations are now superseded by the Events module.
